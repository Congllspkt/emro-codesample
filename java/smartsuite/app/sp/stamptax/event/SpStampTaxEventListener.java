...
@Service
@Transactional
public class SpStampTaxEventListener {

	@Inject
	SpStampTaxService stampTaxService;
	 ...
	 * @param event
	 */
	@EventListener(condition = "#event.eventId == 'findSpListStampTaxPayHistory'")
	public void findSpListStampTaxPayHistory(CustomSpringEvent event) {
		...
		if(Strings.isNullOrEmpty(cntrUuid)) {
			throw new CommonException(this.getClass() + " findSpListStampTaxPayHistory EventListener :" + "There is no cntr_uuid");
		}

		if(Strings.isNullOrEmpty(cntrrTypCcd)) {
			throw new CommonException(this.getClass() + " findSpListStampTaxPayHistory EventListener :" + "There is no cntrr_typ_ccd");
		}
