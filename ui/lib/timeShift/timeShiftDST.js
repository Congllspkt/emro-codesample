...
(function () {
	...
	// 신규 추가
	var timezoneId;
	var timezoneFormatter;
	var cacheMap = {};
	var getTimezoneOffsetDynamic = function (timeParam) {
		// 1. timezoneId가 설정되어 있지 않으면 기존 timezoneOffset을 사용한다.
		if (timezoneId === undefined || timezoneId === null) {
			return timezoneOffset;
		}

		// 2. timezoneId를 사용한다.
		return getTimezoneOffsetByTimezoneId(timeParam);
	}
	function parseTimezoneOffset(offset) {
		// ['', '+2:20', '+9', '-1:10', '-2'];
		if (!offset) return 0;
		var sign = offset[0] === '-' ? -1 : 1;
		var num = offset.substring(1).split(':');
		num = (parseInt(num[0] || 0) * 60 + parseInt(num[1] || 0)) * -sign;
		return num;
	}
	function getTimezoneOffsetByTimezoneId(timeParam) {
		if (timeParam === undefined) {
			timeParam = new OriginalDate(); // timeParam이 없는 경우 현재 기준으로
		}else if(timeParam == "Invalid Date"){
			return isNaN;
		} else if (!(timeParam instanceof OriginalDate || timeParam instanceof TimeShift.Date)) {
			timeParam = new OriginalDate(timeParam);
		}

		// timezoneId는 TimeShift.setTimezoneId으로 지정한 값이다.
		// cache
		if (cacheMap[timezoneId].timestamp === timeParam.getTime()) {
			return cacheMap[timezoneId].offset;
		}

		var parts = timezoneFormatter.formatToParts(timeParam);
		var timeZoneName = parts.find(part => part.type === 'timeZoneName').value; // ['GMT', 'GMT+2:20', 'GMT+9', 'GMT-1:10', 'GMT-2'];
		var offset = parseTimezoneOffset(timeZoneName.replace('GMT', ''));

		// save cache
		cacheMap[timezoneId] = {
			timestamp: timeParam.getTime(),
			offset: offset
		}
		return offset;
	}
	...
	// 외부에서 호출시
	TimeShift.getTimezoneOffsetNameByTimezoneId = function (timezoneId, timeParam) {
		if (timeParam === undefined) {
			timeParam = new OriginalDate();
		} else if (!(timeParam instanceof OriginalDate || timeParam instanceof TimeShift.Date)) {
			timeParam = new OriginalDate(timeParam);
		}
		try {
			...
			var parts = Formatter.formatToParts(timeParam);
			var timeZoneName = parts.find(part => part.type === 'timeZoneName').value;
			return timeZoneName;
		} catch (e) {
			return 'Invalid'; // timezoneId는 해당 브라우저에서 지원하지 않음. 
		}
	}
	...
	function realLocalToUtc(realLocal) {
		return new OriginalDate(realLocal.getTime() - realLocal.getTimezoneOffset() * 60 * 1000 + getTimezoneOffsetDynamic(realLocal) * 60 * 1000);
	}
	function utcToLocal(utc) {
		return new OriginalDate(utc.getTime() - getTimezoneOffsetDynamic(utc) * 60 * 1000);
	}
	function localToUtc(local) {
		return new OriginalDate(local.getTime() + getTimezoneOffsetDynamic(local) * 60 * 1000);
	}
	function twoDigit(n) {
		if (n < 10) {
			return "0" + n;
	...
	function timezoneName() {
		var zone = "GMT";
		var offset = Math.abs(getTimezoneOffsetDynamic());
		if (getTimezoneOffsetDynamic() < 0) {
			zone = zone + "+";
		} else if (getTimezoneOffsetDynamic() > 0) {
			zone = zone + "-";
		} else {
			return zone;
		}
	...
	TimeShift.getTimezoneOffset = function () {
		return getTimezoneOffsetDynamic();
	}
	...
	TimeShift.setTimezoneId = function (id) {
		timezoneId = id;
		try {
			timezoneFormatter = new Intl.DateTimeFormat([], {
				timeZone: timezoneId,
				timeZoneName: 'shortOffset'
			});
		} catch (e) {
	...
	TimeShift.Date.prototype.getTimezoneOffset = function () { return getTimezoneOffsetDynamic(); }
	...
	TimeShift.Date.prototype.desc = function () {
		return "utc=" + this.utc.toUTCString() + "   local=" + utcToLocal(this.utc).toUTCString() + "   offset=" + getTimezoneOffsetDynamic(this.utc);
	}
